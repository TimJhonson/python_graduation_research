...
def process(i,j):
    ...
    i_name = str(round((i-1)*0.0005,4))
    j_name = str(round((j-1)*0.0005,4))

            
    if i != 1 and j != 1:
         
        HS = pd.read_csv('C:/Users/Tim/Desktop/理論數據/Theory/HS/Co_210325_004_HS_728_XMCD_Hex' + j_name + '0' + '_XAS.dat', sep="\s+",    encoding='utf-8',skiprows=6,engine='python',header=None ) 
    ...
    plt.plot(LSx_x,STOy,label= 'theory LV of LS Hex'+ i_name + ' HS' + j_name,color='b')
    plt.plot(LSx_x,STOz,label= 'theory LH of LS Hex'+ i_name + ' HS' + j_name,color='r')
    ...
    plt.xlim(-10,20)
    plt.title( 'STO XMCD 76%LS+24%HS LSHex' + i_name + ' HSHex' +j_name )
    plt.savefig(name + '/' + '728_XMCD_LSHex' + i_name + '_HSHex' + j_name + '.png')
...
for j in range(1, G+1):
    ...
    for i in range(1,M+1):
        ...
        i_name = str(round((i-1)*0.0005,4))
        j_name = str(round((j-1)*0.0005,4))
        path = img_path + '728_XMCD_LSHex' + i_name + '_HSHex' + j_name + '.png'
        tmp = cv2.imread(path)
        vimgs.append(tmp)
